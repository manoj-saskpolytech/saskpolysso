...
import { initialDataSignIn } from "../Login/initialDataSignIn";
import * as validationLib from "../Login/validationAccountSignIn";
...
const LoginComponent: React.FC = () => {
  ...
  const [showDialogForOTPGenerate, setshowDialogForOTPGenerate] = useState(true);
  ...
  const formik = useFormik<IDocument>({
    ...
    validate: (values) => {
      return validationLib.validateSignIn(values,showDialogForOTPGenerate,showDialogForForgetPass);
    },
    onSubmit: async (values, { setSubmitting, resetForm }) => {
      try {
        const result = await DocumentService.getInstance().loginExitAccount(
          formik
        );
        const errorMessages = [
        ...
        if (errorMessages.includes(result)) {
          ...
          setShowToast(true);
          seterrorToastMessage(generateLocalizationMsg("login_success","successfully Logged In"));
          // Inside Account App after login
window.onload = function() {
  const authData = {
    event: 'login',
    token: localStorage.getItem('CognitoIdentityServiceProvider.670obn8euu81d8bu920631g67i.01abe5e0-d061-7015-7f05-129bea6ece06.accessToken'), // or however you're storing it
  };
  window.opener.postMessage(authData, 'https://analytics.proxikle.xyz');
};
        ...
        seterrorToastMessageType("error");
        seterrorToastMessage(generateLocalizationMsg("error_while_login","An unexpected error occurred. Please try again."))
        setShowToast(true);
  ...
  const handleForgetPassEmailSend = async (
    ...
    try {
      ...
      const errorMessages = ["username is required to resetPassword","User does not exist or User Not Verified",'Attempt limit exceeded, please try after some time.'];
      if (errorMessages.includes(result)) {
        setShowToast(true);
        seterrorToastMessageType("error");
        seterrorToastMessage(result);
        ...
        setShowToast(true);
        seterrorToastMessageType("success");
        seterrorToastMessage(generateLocalizationMsg("otp_success",("OTP successfully sent to your mail ID")));
  ...
  const handleNewPasswordSubmit = async (
    ...
    try {
      setLoading(true);
      const isValidOtp = await DocumentService.getInstance().confirmNewPassword(
        Account_Email_ForgetPass,
        Account_OTP,
      ...
      if (errorMessages.includes(isValidOtp)) {
        ...
        setShowToast(true);
        seterrorToastMessageType("success");
        seterrorToastMessage(generateLocalizationMsg("password_changed","New password changed successfully"));
      ...
      seterrorToastMessage(
        error.message || generateLocalizationMsg("error_while_login","An error occurred during OTP validation")
      );
    }
  };
  ...
  return (
    <ThemeProvider theme={theme}>
      ...
      >
      <div style={{display:"flex",justifyContent:"end",alignItems:"center",height:"19px"}}>
            ...
            <Grid item xs={12} md={6}>
              ...
              >
                <Typography
                  variant="h2"
                  component="div"
                  sx={{ display: "flex", alignSelf: "self-start" }}
                >
                  <img
                    src="https://proxiklestatic.com/website/header_footer/proxikle_logo.svg"
                    alt="Proxikle Logo Not Found"
                    style={{
                      marginRight: "5px",
                      width: "145px",
                      height: "25px",
                ...
                <Box
                  sx={{ lineHeight: "40px",width:"100%" }}
                >
                  {
                    <ConfirmationDialog
                      title={generateLocalizationMsg("forgot_password","Forgot Password")}
                      ...
                      SubmitOnClick={() => {
                        handleForgetPassEmailSend(
                          formik.values.Account_Email_ForgetPass
                        );
                      } }
                      readOnly={false}
                      formik={formik}
                      onClose={() => setshowDialogForOTPGenerate(true)}
                      CloseVisibleIcon={true}
                      oktext={generateLocalizationMsg("send_otp", "Send OTP")}
                      fieldHidden={true}  
                   ...
                   {
                    <ConfirmationDialog
                      title={generateLocalizationMsg("forgotpassword_Title","Create New Password")}
                      // message={generateLocalizationMsg("otp_title","A one-time password will be sent to your registered email address for verification.")}
                      message={generateLocalizationMsg("otp_send_changepassword","A one-time password will be sent to your registered email address for verification.")}
                      hidden={showDialogForForgetPass}
                      SubmitOnClick={() => {
                        handleNewPasswordSubmit(
                          formik.values.Account_Email_ForgetPass,
                          formik.values.Account_OTP,
                          formik.values.Account_Password_newPassword
                        );
                      }}
                      formik={formik}
                      onClose={() => setshowDialogForForgetPass(true)}
                      CloseVisibleIcon={true}
                      oktext={generateLocalizationMsg("verify","Verify")}
                      fieldHidden={true}
                      readOnly={true}
                      timerText={timer}
                      closeText={generateLocalizationMsg("resend_otp","Resend OTP")}
                      ...
                      id="Account_OTP"
                      name="Account_OTP"
                      Placeholder={generateLocalizationMsg("enter_otp","Enter OTP")}
                    />
                  }
                  ...
                  {generateLocalizationMsg("email_id","Email")}
                    <span className="requiredLabel">*</span>
                  </FormLabel>
                  <CustomTextField
                    id="Account_Email"
                    name="Account_Email"
                    placeholder={generateLocalizationMsg("enter_email","Enter Email ID")}
                    formik={formik}
                    InputProps={{
                    style: { height: "36px",color:"#253858",backgroundColor:"#ffffff",padding:"10px",borderRadius:"3px"}}} // Adjust the height here
                    autoComplete="new-password"
                  />
                  <FormLabel className="globalLabelName">
                  {generateLocalizationMsg("password","Password")}
                    {/* <span className="requiredLabel">*</span> */}
                  </FormLabel>
                  <CustomTextField
                   autoComplete="new-password"
                   placeholder={generateLocalizationMsg("enter_password","Enter Password")}
                  ...
                  >
                   {generateLocalizationMsg("log_in","Log In")}
                  </CustomButton>
                  <Box sx={{ display: "flex", justifyContent: "end" }}>
                    <CustomButton
                        ...
                        >
                            ...
                            >
                            {generateLocalizationMsg("forgot_password","Forgot Password?")}
                            </span>
                        </CustomButton>
                  </Box>
          ...
          <img src="https://proxiklestatic.com/products/accounts/microsoft__logo.svg" alt="Microsoft_logo Not Available" className="customSignInIcon" />
                  ...
                  <CustomButton
                        ...
                        >
                            ...
                            >
                              {generateLocalizationMsg("create_account","Create a Proxikle Account")}
                            </span>
                        </CustomButton>
                </Box>
              </Box>
            ...
            >
              <Box>
                ...
                >
                  <Typography
                    variant="h5"
                    sx={{ display: "flex", alignItems: "center", mb: 1,color:"#253858",fontWeight:"bold" }}
                  >
                    <img
                ...
                />{" "}&ensp;
                 { generateLocalizationMsg("feature_1","Mobile-ready for On-the-Go Access.")} 
                  </Typography>
                  <Typography
                    variant="h5"
                    sx={{ display: "flex", alignItems: "center", mb: 1,color:"#253858",fontWeight:"bold" }}
                ...
                />{" "}&ensp;
                  { generateLocalizationMsg("feature_2","Multi-Language Support.")} 
                  </Typography>
                  <Typography
                    variant="h5"
                    sx={{ display: "flex", alignItems: "center", mb: 1,color:"#253858",fontWeight:"bold" }}
                ...
                />{" "}&ensp;
                { generateLocalizationMsg("feature_3","Simple, Intuitive User Interface.")} 
                  </Typography>
                </Stack>
              </Box>
            </Grid>
      ...
      <Typography
        variant="h6"
        sx={{ display: "flex", justifyContent: "center", marginTop: "5.5%" ,color:"#253858"}}
      >
        &copy;&nbsp;{generateLocalizationMsg("footer","2024 Proxikle Inc. All Rights Reserved")}
      </Typography>
      </Box>
