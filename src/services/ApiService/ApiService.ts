...
export class ApiService implements IApiService {
  ...
  public async loadlocalizeConfig(): Promise<void> {
    try {
      ...
      if (storedConfig) {
        try {
          ...
          if (parsedConfig && typeof parsedConfig === 'object') {
            ...
            log(logLevels.INFO, "Loaded localization config from session storage");
          } else {
            throw new Error("Invalid data in session storage");
          }
        } catch (e) {
          log(logLevels.ERROR, "Error parsing session storage data:", e);
        }
      }

      if (!apiData) {
        log(logLevels.INFO, "Fetching localization config from API");
        const response = await axios.get('https://zgkys64801.execute-api.us-east-2.amazonaws.com/languages/accounts');
      ...
      const config = {};
      const browserLanguage = await this.getLocConfigJSON("BrowserLangConfig");
  ...
  public getLocConfigValue(configname: string): string {
    try {
      ...
      log(logLevels.DEBUG, `Retrieved loc config value for: ${configname}`);
      return value;
    } catch (error) {
      log(logLevels.ERROR, `Error getting loc config value for: ${configname}`, error);
  ...
  public getLocConfigJSON(configname: string): any {
    try {
      ...
      log(logLevels.ERROR, `Error getting loc config JSON for: ${configname}`, error);
      return {};
    }
  }
  ...
  public getConfigJSON(configname: string): any {
    try {
      ...
      if (this.appConfig[configname]) {
        ...
        result = JSON.parse(this.appConfig["JSONFileContainerObject"] || "{}");
      }
      ...
      log(logLevels.ERROR, `Error getting config JSON for: ${configname}`, error);
      return {};
    }
  }
  ...
  public getConfigValue(configname: string): string {
    try {
      ...
      log(logLevels.ERROR, `Error getting config value for: ${configname}`, error);
      throw new Error("Failed to get config value");
    }
  }

  public async checkIfUserExists(email: string, userPoolId: string): Promise<any> {
    if(_.isEmpty(email)){
    ...
    try {
      ...
      if ((response.Users && response.Users.length > 0)) {
        ...
        const emailVerifiedAttribute = user.Attributes?.find(attr => attr.Name === 'email_verified');
        const emailVerified = emailVerifiedAttribute ? emailVerifiedAttribute.Value === 'false' ? false : true : false;

        log(logLevels.INFO, `User existence check for ${email}: ${emailVerified ? 'Verified' : 'Not Verified'}`);
      ...
      log(logLevels.ERROR, `Error checking if user ${email} exists:`, error);
      throw error;
    }
  }
  ...
  public async checkIfUserNameExist(userpooId: string, UserName: string): Promise<boolean> {
    ...
    try {
      ...
      const responseUserName = await this.cognitoClient.send(UserNamecommand);
      const exists = !!(responseUserName.Users && responseUserName.Users.length > 0);
      
      this.configCache.set(cacheKey, exists);
      log(logLevels.INFO, `Username ${UserName} ${exists ? 'already exists' : 'is available'}`);
      return exists;
    } catch(error) {
      log(logLevels.ERROR, `Error checking if username ${UserName} exists:`, error);
      throw error;
    }
  }
  ...
  public async signup(
    ...
    try {
      const userpooId = 'us-east-2_2t4QCvcFe';
        ...
        const result = await Auth.signUp({
          ...
          options: {
            userAttributes: {
              ...
              'custom:NewsLetters': Account_allowsNewsletter.toString(),
            },
            autoSignIn: true,
          },
        });
        log(logLevels.INFO, `User signed up successfully: ${Account_Email}`);
        return JSON.stringify({ result });
      } else {
        log(logLevels.WARN, `User Email or User Name already exists: ${Account_Email}`);
  ...
  public async otpValidation(
    ...
    try {
      ...
      log(logLevels.INFO, `OTP validated successfully for: ${Account_Email}`);
      return result;
    } catch (error: any) {
      log(logLevels.ERROR, `OTP validation error for ${Account_Email}:`, error);
      return error.message;
    }
  }
  ...
  public async resendOTP(Email: string): Promise<any> {
    try {
      ...
      if (userExists) {
        ...
        log(logLevels.WARN, `User does not exist or is not verified: ${Email}`);
        return 'User does not exist or User Not Verified';
      }
    } catch (error: any) {
  ...
  public async forgotPassword(
    ...
    try {
      const userpooId = 'us-east-2_2t4QCvcFe';
      const userExists = await this.checkIfUserExists(Account_Email_ForgetPass, userpooId);
      if (userExists) {
        const result = await Auth.resetPassword({
          username: Account_Email_ForgetPass,
        });
        log(logLevels.INFO, `Password reset initiated for: ${Account_Email_ForgetPass}`);
        return JSON.stringify({ result });
      } else {
        log(logLevels.WARN, `User does not exist or is not verified: ${Account_Email_ForgetPass}`);
        return 'User does not exist or User Not Verified';
      }
    } catch (error: any) {
      log(logLevels.ERROR, `Forgot password error for ${Account_Email_ForgetPass}:`, error);
      return error.message;
    }
  }
  ...
  public async confirmPassword(
    ...
    try {
      ...
      log(logLevels.INFO, `Password reset confirmed for: ${Account_Email_ForgetPass}`);
      return JSON.stringify({ result });
    } catch (error: any) {
      log(logLevels.ERROR, `Confirm password error for ${Account_Email_ForgetPass}:`, error);
      return error.message;
    }
  }
}